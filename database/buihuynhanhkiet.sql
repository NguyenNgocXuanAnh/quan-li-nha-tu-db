...
-- 3a.3: Tìm phòng giam theo loại phòng bắt đầu bằng chữ 'A' (dùng LIKE)
SELECT MaPhong, MaKV, SucChua, LoaiPhong, TrangThai, GhiChu
FROM PHONGGIAM
WHERE LoaiPhong LIKE N'A%';

-- 3a.4: Hiển thị quản ngục có lương trong khoảng 8,000,000 đến 15,000,000 (dùng BETWEEN)
SELECT MaQuanNguc, TenQuanNguc, ChucVu, MaKV, Luong
FROM QUANNGUC
WHERE Luong BETWEEN 8000000 AND 15000000
ORDER BY Luong DESC;

-- 3a.5: Tù nhân nam, đang thi hành án hoặc đang theo dõi, không phải mức nguy hiểm 'Thấp' (dùng AND, OR, NOT)
SELECT MaTuNhan, HoTen, NgaySinh, GioiTinh, MucDoNguyHiem, TrangThai
FROM TUNHAN
WHERE GioiTinh = N'Nam'
  AND (TrangThai = N'Đang thi hành án' OR TrangThai = N'Đang theo dõi')
  AND NOT MucDoNguyHiem = N'Thấp'
ORDER BY MucDoNguyHiem;


-- 3g.1: Cập nhật lương quản ngục QN01 tăng 10%
UPDATE QUANNGUC
...
-- 3g.2: Cập nhật trạng thái phòng PA103 thành 'Đang sử dụng'
UPDATE PHONGGIAM
...
-- 3g.3: Cập nhật trạng thái tù nhân TN003 thành 'Đã mãn hạn'
UPDATE TUNHAN
...
-- 3g.4: Cập nhật hình thức xử lý vi phạm VP001 thành 'Lao động công ích'
UPDATE VIPHAMKYLUAT
...
-- 3g.5: Cập nhật trạng thái tài khoản quản ngục QN16 thành 'Hoạt động'
UPDATE TAIKHOAN
...
-- 3g.6: Xóa lịch thăm nuôi chưa duyệt của tù nhân TN001
...
-- 3g.7: Xóa những công việc không có tù nhân nào tham gia cải tạo
DELETE FROM CONGVIEC
WHERE MaCongViec NOT IN (
    SELECT DISTINCT MaCongViec FROM CAITAO
);

SELECT CV.MaCongViec, CV.TenCongViec, CV.TrangThai,
       COUNT(CT.MaTuNhan) AS SoTuNhanCaiTao
FROM CONGVIEC CV
JOIN CAITAO CT ON CV.MaCongViec = CT.MaCongViec
GROUP BY CV.MaCongViec, CV.TenCongViec, CV.TrangThai;


-- Thủ tục 1: Tìm kiếm tù nhân theo tên
GO
...
-- Ví dụ: EXEC sp_TimKiemTuNhan N'Nguyễn';
...
-- Ví dụ: EXEC sp_ThemViPham 'VP021', 'TN010', '2026-05-01', N'Gây rối trong giờ nghỉ', N'Cảnh cáo', N'Vi phạm lần đầu';


-- Hàm 1 (Scalar): Đếm số lần vi phạm của một tù nhân, trả về giá trị INT
GO
...
-- Ví dụ: SELECT dbo.fn_DemViPham('TN004') AS SoLanViPham;

-- Hàm 2 (Table-valued): Lấy danh sách tù nhân trong một phòng giam, trả về bảng
GO
...
-- Ví dụ: SELECT * FROM dbo.fn_DanhSachTuNhanTheoPhong('PC101');


-- Trigger: Tự động cập nhật TrangThai phòng giam khi tù nhân chuyển phòng
GO
...
-- USER 1: admin_qlnt - Quản trị viên hệ thống (toàn quyền)
...
-- USER 5: bao_ve - Nhân viên bảo vệ (chỉ xem)
